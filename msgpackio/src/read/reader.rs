...
pub trait MsgPackReader : Read
{
	...
	fn read_msgpack_value(&mut self) -> Result<Value>
	{
		let mut buf : [u8;1] = [0];
		...
		match msgpack_id
		{
			MsgPackId::Nil => { return Ok(Value::Nil); }	
			
			//Boolean
			MsgPackId::False => { return Ok(Value::Boolean(false)); }
			MsgPackId::True => { return Ok(Value::Boolean(true)); }
			
			//fixed integer
			MsgPackId::FixPosInt => { return Ok(Value::UInt8(buf[0] & 0x7f)); }
			MsgPackId::FixNegInt => { return Ok(Value::Int8(-((buf[0] & 0x1f) as i8))); } //right?
			...
			*/
			...
			MsgPackId::FixStr => { 
				...
				if let Ok(str) = String::from_utf8(buf) {
					return Ok(Value::Str(msgpack_id, str));
				}
				else {
			...
			MsgPackId::Bin8 => {
				...
				let mut buf = Vec::with_capacity(len);
				try!(self.read(&mut buf));
				return Ok(Value::Bin(msgpack_id, buf));
			}
			...
			MsgPackId::Ext8 => {
				...
				let mut buf = Vec::with_capacity(len);
				try!(self.read(&mut buf));
				return Ok(Value::Ext(msgpack_id, ty, buf));
			}
